...
def enable_llama_training(
    ...
    for idx, layer in enumerate(model.model.layers):
        ...
        module.register_parameter(
            "scaling_factors",
            nn.Parameter(
                (scaling_factors[idx].reshape(model.config.num_key_value_heads, module.head_dim).to(device, dtype))
...
def set_scaling_factors_llama(model, scaling_factors):
    ...
    if isinstance(model, LlamaForCausalLM):
        ...
        for layer_idx, layer in enumerate(model.layers):
            ...
            mesh = DeviceMesh(device_type="cuda", mesh=[i for i in range(int(os.environ["WORLD_SIZE"]))])
