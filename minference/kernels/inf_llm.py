...
class ContextManager:
    ...
    def init(
        ...
        if self.dense_decoding:
            self.dense_k = torch.empty((self.num_units, self.unit_size_kv, 0, dim_head), dtype=local_k.dtype, device=local_k.device)
            self.dense_v = torch.empty((self.num_units, self.unit_size_kv, 0, dim_head), dtype=local_v.dtype, device=local_v.device)
...
class InfLLMGenerator(GreedySearch):
    def generate(self, input_ids=None, generation_config=None, pad_token_id=None, max_new_tokens=None):
        if max_new_tokens is not None:
            max_new_tokens = max_new_tokens
        else:
            max_new_tokens = generation_config.max_new_tokens
        return super().generate(
            text=None, input_ids=input_ids,
