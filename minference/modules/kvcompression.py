...
def snapkv_forward(
    ...
    if query_states.shape[-1] != cos.shape[-1]: # glm-4 rope
        query_states, key_states = apply_rotary_pos_emb_glm4(query_states, key_states, cos, sin)
    else:
    ...
    if past_key_value is not None:
        ...
        key_states, value_states = past_key_value.update(  # kvcompress
            query_states, key_states, value_states, attention_mask, self.layer_idx,
            self.num_key_value_groups, cache_kwargs
        )
