...
class VectorDB_KV_Cache():
    ...
    def async_update(self, 
        ...
        assert self.start_size+self.recent_size==self.cache_size    # 检查大小正确
        ...
        if layer_idx <= do_strllm_until_layer:
            ...
            assert 0<cache_size<=qlen # 有pattern时cache_size不能为0

            key_tensor = torch.zeros((bs, head_num, cache_size, dim), dtype=key_states.dtype, device=key_states.device)
            value_tensor = torch.zeros((bs, head_num, cache_size, dim), dtype=key_states.dtype, device=key_states.device)
            
            key_tensor[:, :, :, :] = torch.cat((key_states[:, :, :start_size, :], key_states[:, :, qlen-recent_size:qlen, :]), dim=2)
            value_tensor[:, :, :, :] = torch.cat((value_states[:, :, :start_size, :], value_states[:, :, qlen-recent_size:qlen, :]), dim=2)
        ...
        self.temp_gpu_key_cache.append(torch.zeros((bs, head_num, self.temp_cache, dim), dtype=key_states.dtype, device=key_states.device))
        self.temp_gpu_value_cache.append(torch.zeros((bs, head_num, self.temp_cache, dim), dtype=key_states.dtype, device=key_states.device))
        ...
        self.update_thread = threading.Thread(target=update_task, args=(key_states, value_states, layer_idx))
    ...
    # sync更新，包了一层load data
    def sync_key_update(self, 
        key_states, 
        layer_idx,
        do_strllm_until_layer,
        core,
        ...
        if insert_db:
            ...
            if layer_idx <= do_strllm_until_layer and recent_size > 0:
                ...
                key_tensor = torch.zeros((bs, head_num, 1, dim), dtype=key_states.dtype, device=key_states.device)
                key_tensor = self.gpu_key_cache[layer_idx][:, :, start_size:start_size+1, :]
                tempk = torch.cat([self.gpu_key_cache[layer_idx][:, :, :start_size, :], self.gpu_key_cache[layer_idx][:, :, start_size+1:, :]], dim=-2)
            ...
            if layer_idx == 0:
                self.temp_seen+=1
            self.temp_gpu_key_cache[layer_idx][:,:,self.temp_seen-1:self.temp_seen,:] = key_states
            return self.key_cache[layer_idx], torch.cat((self.gpu_key_cache[layer_idx], self.temp_gpu_key_cache[layer_idx][:,:,:self.temp_seen,:]), dim=-2)
            
    
    def sync_value_update(self, 
        value_states, 
        layer_idx,
        do_strllm_until_layer,
        insert_db
        ...
        if insert_db:
            ...
            if layer_idx <= do_strllm_until_layer and recent_size > 0:
                ...
                value_tensor = torch.zeros((bs, head_num, 1, dim), dtype=value_states.dtype, device=value_states.device)
                value_tensor = self.gpu_value_cache[layer_idx][:, :, start_size:start_size+1, :]
                tempv = torch.cat([self.gpu_value_cache[layer_idx][:, :, :start_size, :], self.gpu_value_cache[layer_idx][:, :, start_size+1:, :]], dim=-2)
                self.gpu_value_cache[layer_idx] = torch.cat([tempv, value_states], dim=-2)
            else:
                value_tensor = value_states
    
            value_states = value_tensor.detach().cpu()
            value_cache = self.val_vec_cache_update(value_states, layer_idx, do_strllm_until_layer)
            return value_cache, self.gpu_value_cache[layer_idx]
        else:
            self.temp_gpu_value_cache[layer_idx][:,:,self.temp_seen-1:self.temp_seen,:] = value_states
            return self.value_cache[layer_idx], torch.cat((self.gpu_value_cache[layer_idx], self.temp_gpu_value_cache[layer_idx][:,:,:self.temp_seen,:]), dim=-2)

    
     # papyfaiss 底层vectordb cache更新
    def key_vec_cache_update(
            self,
            key_states,
            layer_idx,
            core
        ...
        if len(self.key_cache) <= layer_idx:
            # Flat index
            index = papyfaiss.FlatIndex(head_num=key_states.shape[1], dim=key_states.shape[3])
...
class RetrAttnCache(Cache):
    def __init__(self, config):
        ...
        self.vector_db_cache = VectorDB_KV_Cache(
            self.max_length, self.temp_cache_size
        )
    ...
    def update(
        ...
        if key_states.size(-2) != 1:  # prefill
            self.vector_db_cache.async_update(
                key_states.contiguous(), value_states, layer_idx, self.do_strllm_until_layer, self.core
            )
...
def retr_attn(
    ...
    num_key_value_groups = decoding_kwargs.get("num_key_value_groups", q.size(1) // gpu_key_cache.size(1))
    ...
    if key_cache.get_index_type() == "Flat": 
        ...
        distances, indices = key_cache.search(q[0], layer_idx, num_key_value_groups, top_k, core)

    retrieval_attn_weights = distances.reshape((q.shape[0], q.shape[1], q.shape[2], top_k)) / math.sqrt(q.shape[3])
    ...
    retrieval_lse = torch.log(torch.sum(torch.exp(retrieval_attn_weights), dim=-1)).to(device)
    retrieval_attn_weights = nn.functional.softmax(retrieval_attn_weights, dim=-1, dtype=torch.float32)
    retrieval_attn_weights = nn.functional.dropout(retrieval_attn_weights, p=0, training=False)
    
    retrieval_out = torch.full((q.shape[0], q.shape[1], q.shape[2], q.shape[3]), 0, dtype=torch.float32)
    for head_idx in range(q.shape[1]):
        for q_idx in range(q.shape[2]):
            query2key = head_idx // num_key_value_groups
            retrieval_attn_values = value_cache[0][query2key][retrieval_indices[head_idx][q_idx]] # (bs, nhead, true_k, head_dim)
            retrieval_out[:, head_idx:, q_idx, :] = torch.matmul(retrieval_attn_weights[:, head_idx, q_idx, :], retrieval_attn_values.to(torch.float32))
...
def llama_retr_flash_attention_forward(
    ...
    **kwargs,
    ...
    # prefill阶段, 纯flash attn
    if key_states.shape[2] != 1:
        # (1) 异步更新
        # past_key_value.sync_key_update(key_states, self.layer_idx)
        # past_key_value.sync_value_update(value_states, self.layer_idx)
        past_key_value.async_update(key_states.contiguous(), value_states, self.layer_idx, do_strllm_until_layer, core)

        # (2) 计算
        attn_output = flash_attn(query_states, key_states, value_states, attention_mask, q_len, causal=True, dropout=dropout_rate)
    ...
    # decoding阶段    
        ...
        key_cache, gpu_key_cache = past_key_value.sync_key_update(key_states, self.layer_idx, do_strllm_until_layer, core, insert_db)
        value_cache, gpu_value_cache = past_key_value.sync_value_update(value_states, self.layer_idx, do_strllm_until_layer, insert_db)

        attn_output = retr_attn(query_states, key_cache, value_cache, gpu_key_cache, gpu_value_cache, self.num_key_value_groups, self.layer_idx, top_k, from_layer, core)
