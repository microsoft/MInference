...
def convert_indices(
    ...
    #     torch.save(block_mask, f'./output/data/block_mask_{rank}.pt')
    #     torch.save(bar_idx, f'./output/data/bar_idx_{rank}.pt')
    #     torch.save(bar_cnt, f'./output/data/bar_cnt_{rank}.pt')
    # elif rank == 0:
    #     torch.save(block_mask, f'./output/data/block_mask.pt')
    #     torch.save(bar_idx, f'./output/data/bar_idx.pt')
    #     torch.save(bar_cnt, f'./output/data/bar_cnt.pt')
    # bar_cnt = torch.zeros_like(bar_cnt)
...
def calc_index(
    ...
    if os.getenv("COMM_DEBUG", False):
        # For debugging purposes, print the rank and tensor shapes
        rank = dist.get_rank(group)
        print(f"Rank {rank} | calc_index | before invoking broadcast s_indices from rank={s_gather_rank}", flush=True)
...
def compute_sr_flops(
    ...
    bar_cnt_offset: torch.Tensor,  # [batch_size, num_qo_heads, num_blocks]
    granularity: int,
    q_len: int,
    head_dim: int,
    shift: bool,
    ...
    block_ratio = block_mask_offset.sum(dtype=torch.float32).item() / total_num_blocks
    bar_ratio = bar_cnt_offset.sum(dtype=torch.float32).item() / (granularity * total_num_blocks)
    sparsity_ratio = 1 - block_ratio - bar_ratio

    block_flops = block_mask_offset.sum(dtype=torch.float32).item() * (granularity * granularity) * head_dim * 2 * 2
    bar_flops = bar_cnt_offset.sum(dtype=torch.float32).item() * granularity * head_dim * 2 * 2
...
def compute_sr_by_heads(
    ...
    num_heads = block_mask_offset.shape[1]
    num_blocks = triton.cdiv(q_len, granularity)
    if not shift:
    ...
    block_ratio_by_heads = block_mask_offset.sum(dim=-1).sum(dim=-1).sum(0, dtype=torch.float32) / total_num_blocks_by_heads
    bar_ratio_by_heads = bar_cnt_offset.sum(dim=-1).sum(0, dtype=torch.float32) / total_num_blocks_by_heads / granularity
    sparsity_ratio_by_heads = 1 - block_ratio_by_heads - bar_ratio_by_heads

    return sparsity_ratio_by_heads

def get_compute_sparsity(
    q: torch.Tensor,
    k: torch.Tensor,
    batch_size: int,
    num_tokens: int,
