...
def patch_glm_4_1m(model):
    ...
    def patch_forward(model):
        if model.__class__.__name__ == "ChatGLMForConditionalGeneration":
            ...
            model.prepare_inputs_for_generation = types.MethodType(LlamaForCausalLM.prepare_inputs_for_generation, model)
            model._update_model_kwargs_for_generation = types.MethodType(LlamaForCausalLM._update_model_kwargs_for_generation, model)

        for name, module in reversed(model._modules.items()):
            if len(list(module.children())) > 0:
                patch_forward(module)
            if module.__class__.__name__ == "ChatGLMModel":
                model._modules[name].forward = types.MethodType(LlamaModel.forward, model._modules[name])
                # model._modules[name].rotary_emb.forward = types.MethodType(rope_forward, model._modules[name].rotary_emb)
                model._modules[name].rotary_emb = GlmRotaryEmbedding(model._modules[name].rotary_emb.dim, model.config.seq_length)
                model._modules[name]._update_causal_mask = types.MethodType(LlamaModel._update_causal_mask, model._modules[name])
            if module.__class__.__name__ == "ChatGLMPreTrainedModel":
                model._modules[name].forward = types.MethodType(LlamaPreTrainedModel.forward, model._modules[name])
            if module.__class__.__name__ == "GLMBlock":
                model._modules[name].forward = types.MethodType(LlamaDecoderLayer.forward, model._modules[name])
            if module.__class__.__name__ == "SelfAttention":
                model._modules[name].forward = types.MethodType(LlamaAttention.forward, model._modules[name])
            if module.__class__.__name__ == "FlashAttention2":
                model._modules[name].forward = types.MethodType(LlamaFlashAttention2.forward, model._modules[name])
