...
from mtraining_sparse_ops import get_minference_config, minference_flash_attn_func
MINFERENCE_CONFIG = get_minference_config("Qwen2.5_3B_kv_out_v32_fit_o_best_pattern.json")
...
class Qwen2FlashAttention2(Qwen2Attention):
    ...
    def forward(
        ...
        if query_states.shape[1] == key_states.shape[1]:  # Prefilling
            v_size, s_size = MINFERENCE_CONFIG[self.layer_idx]
            attn_output = minference_flash_attn_func(query_states, key_states, value_states, v_size, s_size)
        else:
            attn_output = flash_attn_with_kvcache(query_states, key_states, value_states)
